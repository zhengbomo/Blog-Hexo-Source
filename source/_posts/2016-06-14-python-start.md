...
## 数据类型
与其他高级语言一样，python支持基础数据类型，也支持高级数据类型
python支持如下数据类型
...
## 支持切片
squares[a:b:c]          # 表示从下标a到下标b且不包含下标b，取步长为c的集合
...
### 元组tuple
元组相当于多维数组，用括号表示，括号有时可以省略（不产生歧义的情况下）
...
__reduce__  
首先操作前两个数，然后结果与第三个数操作，以此类推：`reduce(function, sequence)`
```python
def add(x,y):
  return x+y
...
> 多继承：如果子类有自己实现的构造函数，同单继承，不会主动调用父类的构造函数，如果子类没有实现自己的构造函数，则会从父类中优先选择有构造函数的父类（有问题）
...
```python
try:
    raise Exception('spam', 'eggs')   # 手动跑出异常
